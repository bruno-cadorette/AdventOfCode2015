...
applySpell (p, m) (Shield s) = (p {playerArmor = playerArmor p + s}, m)
...
removeEffect (p, m)  (Shield s) = (p {playerArmor = playerArmor p - s}, m)
removeEffect f _ = f
...
spellEffect f magic = (fight, m)
    where 
        ...
        fight = applySpell (if isJust m then f else (removeEffect f (spell magic))) (spell magic)
...
traceHistory :: Player -> a -> a
traceHistory p = traceShow (map spell $ reverse $ magicHistory p)
